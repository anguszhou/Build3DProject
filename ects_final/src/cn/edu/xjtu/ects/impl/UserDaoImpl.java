...
public class UserDaoImpl implements IUserDao {
	...
	public Admin loginadmin(String name, String passwd) throws ManagerUserException {
		Session session = null;
		Transaction  tran = null;
		Admin admin = null;
		try{
			session = HibernateUtils.getSession();
			tran = session.beginTransaction();
			admin=(Admin) session.createQuery(" from  Admin where name=:name and passwd =:passwd ").setString("name",name).setString("passwd",passwd).uniqueResult();
			tran.commit();
		}catch(Exception e){
			tran.rollback();
			e.printStackTrace();
