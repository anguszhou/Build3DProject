...
IndexEvent.prototype = {
	...
	getIndexWeather : function(){
		...
		nova_update('weather','/index_weather/');
	},
	/**
	 * 登录成功后的 显示会员信息面板
	 */
	indexLoginTask : function(){
		...
		nova_update("index_login_div","/index_prodad/");
	},
	...
	showVTip : function (evt){
		...
		if(evtElm.className.indexOf('show') >= 0)
			evtElm.down('.show_update').show();
	},
	hideVTip : function (evt){
		...
		if(evtElm && evtElm.className.indexOf('show') >= 0)
			evtElm.down('.show_update').hide();
	},
	keepVTip : function () {
	...
	cookielistInit: function(id) {
		...
		var delOne = $A(this.con.getElementsByClassName('del'));
		this._PlaylistDel = this.PlaylistDel.bindAsEventListener(this);
		delOne.each(function(o){
			Element.extend(o);
			if(o.nodeName == 'SPAN' && o.readAttribute('observe') != '1') {
	...
	PlaylistDel: function(evt) {
		...
		if(elm.getAttribute('handler') == 'playlist') {
			...
			if(list.getElementsByTagName('li').length >= 20)
				return nova_update('index_cookielist_div', '/index_cookielist/');
			elm.up().remove();
	...
	pager: function(event, dir, con) {
		var pages = con.getElementsByClassName('items');
		var i = pages.length;
...
window.nova_init_hook_event = function(){
	// 初始化 首页登录/会员信息 区域
	if(islogin()){
		nova_update("index_login_div","/index_prodad/");         // 会员个人信息
	}

	nova_update("index_cookielist_div","/index_cookielist/");         // 会员个人信息

	QIndex.getIndexWeather();
