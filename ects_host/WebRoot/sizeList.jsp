       ...
       <!-- 主体内容 -->
        ...
        <div class="right_content">
            ...
            <div class="pagination">
            	...
            	<br/><br/>
                <a href="${pageContext.request.contextPath}/sizeList.do?destId=1&size=${size}">首页</a>
                <c:if test="${pageController.hasPrevious}">
            		<a href="${pageContext.request.contextPath}/sizeList.do?destId=${pageController.currentPage-1}&size=${size}">上一页</a>
            	</c:if>
            		<span class="current">
						<select id="pageSelector" style="font-size:12px;padding-top:3px;"
							onchange="location='${pageContext.request.contextPath}/sizeList.do?destId='+this.value+'&size=${size}'">
							<c:out value="${pageSelector}" escapeXml="false"></c:out>
						</select>
					</span>				
				<c:if test="${pageController.hasNext}">
            		<a href="${pageContext.request.contextPath}/sizeList.do?destId=${pageController.currentPage+1}&size=${size}">下一页</a>
            	</c:if>
            	<a href="${pageContext.request.contextPath}/sizeList.do?destId=${pageController.totalPages}&size=${size}">最后一页</a>
            </div> 
        	<div class="clear"></div>
        </div>
      <!-- 右侧栏 -->
