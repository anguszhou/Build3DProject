...
export PATH=/usr/kerberos/sbin:/usr/kerberos/bin:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin:/sbin/nologin
...
EXTRACT_FOCAL=$BASE_PATH/runbundler/bin/extract_focal.pl

OS=`uname -o`

if [ $OS == "Cygwin" ]
then
    MATCHKEYS=$BASE_PATH/runbundler/bin/KeyMatchFull.exe
    BUNDLER=$BASE_PATH/runbundler/bin/Bundler.exe
else
    MATCHKEYS=$BASE_PATH/runbundler/bin/KeyMatchFull
    BUNDLER=$BASE_PATH/runbundler/bin/bundler
fi

TO_SIFT=$BASE_PATH/runbundler/bin/ToSift.sh
