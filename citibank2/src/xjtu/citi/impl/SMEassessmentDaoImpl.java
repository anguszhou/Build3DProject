...
public class SMEassessmentDaoImpl implements ISMEassessmentDao {
	...
	public Collection calSMEassess() {
		...
		//计算中小企业的预警信息
		late = session.createQuery("from SMEassessment p1 where p1.time=(select max(p2.time) from SMEassessment p2)").list(); 
		for (Iterator iterator2 = late.iterator(); iterator2.hasNext();)
		{
			...
			if((wi.getPrevalue()-wi.getLatevalue()) >= wp.getValue()*wi.getPrevalue())
			{
				session.save(wi);
			}
		}
	...
	public Collection<SMEassessment> getSMEassess() 
	{
		...
		try 
			{		
			  String hql = "from SMEassessment p1 where p1.time=(select max(p2.time) from SMEassessment p2) order by score desc";		  
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();)
		{
			...
			System.out.println(e.getCompanyinfo().getName());
		}
		return list;
	}
