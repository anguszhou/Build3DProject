...
* @return 生成图片的文件名 
*/ 
public String getScoreChart(HttpSession session, PrintWriter pw , int id) 
{ 
XYDataset dataset = this.createDateSet(id);//建立数据集 
...
//标题
TextTitle title = new TextTitle("企业历史得分曲线", font_title); 
...
numAxis.setAutoRangeIncludesZero(true);//从0开始
...
axis.setTickUnit(new DateTickUnit(DateTickUnit.DAY,2,new SimpleDateFormat("yy年MM月dd日")));//设置坐标间隔
...
TimeSeries s1 = new TimeSeries("企业历史得分曲线",Day.class);//创建时间数据源，每一个//TimeSeries在图上是一条曲线 
...
for (Iterator iterator = list.iterator(); iterator.hasNext();)
{
	...
	s1.add(new Day(sme.getTime()),sme.getScore());
}
